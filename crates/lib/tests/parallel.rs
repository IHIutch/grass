...
fn from_paths_error_does_not_affect_others() {
    let mut good_file = tempfile::Builder::new()
        .suffix(".scss")
        .tempfile()
        .unwrap();
...
fn parallel_uswds_matches_sequential() {
    ...
    let path = &format!("{}/prototype/packages/uswds/_index-direct.scss",
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap().parent().unwrap().display());
    ...
    if sequential != parallel {
        ...
        eprintln!("Size mismatch: seq={} par={} (diff={})", sequential.len(), parallel.len(),
        ...
        for (i, (s, p)) in seq_lines.iter().zip(par_lines.iter()).enumerate() {
            if s != p {
                eprintln!("First diff at line {}: seq={:?}", i+1, &s[..s.len().min(80)]);
