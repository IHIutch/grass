...
pub fn convert(channels: [f64; 3], from: ColorSpace, to: ColorSpace) -> [f64; 3] {
    if from == to {
        return channels;
    }
    ...
    match (from_linear, to_linear) {
        ...
        (Some(from_ls), None) if to == ColorSpace::Oklab || to == ColorSpace::Oklch => {
            let linear = to_linear_rgb(channels, from);
        ...
        (None, Some(to_ls)) if from == ColorSpace::Oklab || from == ColorSpace::Oklch => {
            let [c0, c1, c2] = channels;
...
fn convert_direct(channels: [f64; 3], from: ColorSpace, to: ColorSpace) -> Option<[f64; 3]> {
    ...
    match (from, to) {
        ...
        (ColorSpace::DisplayP3Linear, ColorSpace::DisplayP3) => Some([
            display_p3_encode(c0),
            display_p3_encode(c1),
            display_p3_encode(c2),
        ]),
...
mod tests {
    ...
    fn identity_conversion() {
        ...
        assert_eq!(
            convert(channels, ColorSpace::SRgb, ColorSpace::SRgb),
            channels
        );
    }
